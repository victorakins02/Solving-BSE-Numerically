...
df1 = pd.read_csv('ex-psi-initial-norm.csv')
position1 = df1[df1.columns[0]]
probability_density1 = df1[df1.columns[1]]

df2 = pd.read_csv('psi-t-norm-cn.csv')
position2 = df2[df2.columns[0]]
probability_density2 = df2[df2.columns[1]]

relative_error = [
    abs(probability_density2[i] - probability_density1[i]) / abs(probability_density1[i])
    for i in range(len(probability_density1))
]
